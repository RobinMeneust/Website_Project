	...
	xhttp.onload = function () {
		if(this.readyState == 4 && this.status == 200){
			...
			for (let i = 0; i < name.length; i++) {
				txt += '<tr> <td><img style="width:150px" src="'+imgSrc[i].childNodes[0].nodeValue+'"></td><td>'+id[i].childNodes[0].nodeValue+'</td><td>'+desc[i].childNodes[0].nodeValue+'</td><td>'+price[i].childNodes[0].nodeValue+'</td><td class="stockColumn" id="stockID'+(i+1)+'">'+stock[i].childNodes[0].nodeValue+'</td><td><form action="/action_pageWIP.php"><input type="button" id="button-'+(i+1)+'" value="-" onclick="decrease('+(i+1)+');" disabled="disabled"><input onkeyup="checkIfLesserThanStock(this.value, '+(i+1)+')" type="number" id="quantity'+(i+1)+'" name="quantity" min="0" max="" value="0" size="6"><input type="button" id="button+'+(i+1)+'" value="+" onclick="increase('+(i+1)+');"><br><input type="button" class="button" value="Ajouter au panier"></form></td> </tr>';
			}
