    ...
    xhttp.onload = function () {
        if(this.readyState == 4 && this.status == 200){
            ...
            for (let i = 0; i < name.length; i++) {
                txt += '<tr> <td><img style="width:150px" src="'+imgSrc[i].childNodes[0].nodeValue+'" alt="photo"></td><td>'+id[i].childNodes[0].nodeValue+'</td><td>'+desc[i].childNodes[0].nodeValue+'</td><td>'+price[i].childNodes[0].nodeValue+'</td><td class="colonne">'+stock[i].childNodes[0].nodeValue+'</td><td><form action="/action_page.php"><input type="button" id="button1" value="-" onclick="diminuer(`quantity1`,`button1`)" disabled="disabled"><input type="number" id="quantity1" name="quantity" min="0" max="" value="0" size="6"><input type="button" value="+" onclick="augmenter(`quantity1`,`button1`)"><br><input type="button" class="button" value="Ajouter au panier"></form></td> </tr>';
            }
